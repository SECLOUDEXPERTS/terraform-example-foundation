...
module "gcp_projects_state_bucket" {
  ...
  name          = "${module.seed_bootstrap.seed_project_id}-csb-projects-tfstate"
...
module "tf_source" {
  ...
  project_id            = "${var.project_prefix}-${var.env}-cicd-${random_string.random.result}"
  location              = var.default_region
  ...
  project_labels = var.project_labels_cicd
...
module "tf_cloud_builder" {
  ...
  bucket_name                  = "${module.tf_source.cloudbuild_project_id}-csb-tf-cloudbuilder-build-logs"
}
...
module "tf_workspace" {
  ...
  log_bucket_name           = "${module.tf_source.cloudbuild_project_id}-csb-${local.cb_config[each.key].source}-build-logs"
  artifacts_bucket_name     = "${module.tf_source.cloudbuild_project_id}-csb-${local.cb_config[each.key].source}-build-artifacts"
